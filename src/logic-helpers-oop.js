...
this.questions3 = [
    ...
    { letter: "o", answer: "obsesion", status: 0, question: "With the letter O.\nA state in which someone thinks about someone or something constantly or frequently especially in a way that is not normal."},
...
class AlphabetGame {
  ...
  play() {
    ...
    while(this.randomQuestions.some(elem => elem.status === 0)) {
      this.round++
      console.log("Round " + this.round)
      for(let i = 0; i < this.randomQuestions.length; i++) {
        if(this.randomQuestions[i].status === 0) {
          console.log(this.randomQuestions[i].question)
          let answer = prompt("Insert your answer:");
          if(answer === null || answer.toUpperCase() === "END") {
            console.log("\nOk, the game is over\n");
            return;
          } else if(answer.toUpperCase() === this.randomQuestions[i].answer.toUpperCase()) {
            this.randomQuestions[i].status = true;
            ...
            this.randomQuestions[i].status = false;
            console.log("\nIncorrect!\n");
          }
        }
      }
  ...
  showResults() {
    console.log(this.player.name + ", you finally scored " + this.player.points + " points and it took you " + this.round + " rounds.");
      let rightAnswers = [];
      let wrongAnswers = [];
      this.randomQuestions.forEach(elem =>
  ...
  addToRanking() {
     if(this.randomQuestions.every(elem => elem.status !== 0)) {
  ...
  showRanking() {
    this.ranking.showRanking(this.player)
  }

  getRandomArray(i) {
  let allQuestions = [this.material.questions, this.material.questions2, this.material.questions3]
  ...
  setRandomArray() {
    ...
    for(let i in this.material.questions) {
